...
from keras.layers import Input, Dense, Dropout, Embedding, Bidirectional, LSTM, concatenate
from keras.utils.vis_utils import plot_model
from keras import regularizers
from utils import preprocess_from_df, tokenizer_from_json, clean_text, save_confusion_matrix
...
label_column = 2
sentence_column = 3
labels_to_true = ["true", "half-true", "mostly-true"]
...
def prep_data(filepath, tokenizer):
    ...
    data[label_column] = data[label_column].apply(lambda x: x in labels_to_true).astype(int)
    data[sentence_column] = data[sentence_column].apply(lambda x: clean_text(x))
    data = data[relevant_columns].dropna()

    X, y = preprocess_from_df(data, unique_labels, normalise_columns = columns_to_normalise, label_column = label_column, encode = False, split = False)
    sentences = list(X[:, -1])
    X_meta = X[:,:-1]
...
if __name__ == "__main__":
    ...
    model = None
    if not os.path.isfile('models/nn.json'):
        ...
        plot_model(model, to_file='figures/nn_model_plot.png', show_shapes=True, show_layer_names=True)
        model.fit([X_train_sequences, X_train_meta], y_train, validation_data=([X_val_padded, X_val[:,:-1]], y_val), batch_size = 128, epochs=2)

        model_json = model.to_json()
        with open("models/nn.json", "w") as json_file:
        ...
        json_file = open('models/nn.json', 'r')
        loaded_model_json = json_file.read()
        json_file.close()
        model = model_from_json(loaded_model_json)
        # load weights into new model
        model.load_weights("models/nn_weights.h5")
    ...
    y_pred = [labels[1] if y_ > 0.5 else labels[0] for y_ in model.predict([X_test_sequences, X_test_meta])]
    y_true = labels[y_test]
    
    save_confusion_matrix(y_pred, y_true, labels,'binary')
